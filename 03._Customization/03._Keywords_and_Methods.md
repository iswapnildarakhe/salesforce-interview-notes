...
2. If you have multiple Constructor's make sure they have different signatures.
```apex
public class Car {
    // Properties i.e. Variables
    String typeofCar;
...
**NOTE** : 
1. You only override `virtual` methods. You don't override `abstract` methods. Why? 
Because, `abstract` methods are just method definitions they don't have body and you are mandated to Implement them in the Class which is extending the `abstract` class.
